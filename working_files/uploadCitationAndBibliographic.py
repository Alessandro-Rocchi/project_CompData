...
class CitationUploadHandler(UploadHandler):
    ...
    def pushDataToDb(self, path: str) -> bool: #Takes the path of a CSV in input, transform the data in RDF triple and push them to a graph DB
        ...
        for idx, rows in citations.iterrows():
            if rows["oci"] and rows["citing"] and rows["cited"] and rows["creation"] and rows["timespan"]: #check if there are empty values in the row. If there are empty values, the row is not added to the graph because it is not complete and could cause problems in the query phase.
                ...
                my_graph.add((subj, cited, URIRef(rows["cited"]))) #I add to the graph the relation "hasCitedEntity" between the subject (the citation) and the cited entity (the paper that is being cited). I create a URIRef forthe cited entity usingthe value fromthe "cited" column inthe CSV file. This allows us to representthe relationship betweenthe citationandthe cited entity inthe RDF graph, which can be useful for querying and analyzing citation patterns.
